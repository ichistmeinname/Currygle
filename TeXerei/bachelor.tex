...
\\
\>[B]{}\Varid{sigReservedOp}\mathrel{=}\Varid{reservedOp}\;\Varid{signatureTokenParser}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
and parses surrounding white spaces; \ensuremath{\Varid{aSymbol}} does the same, but
...
\\
\>[3]{}\hsindent{9}{}\<[12]%
\>[12]{}~\rotatebox[origin=c]{90}{$\ominus$}~\Varid{aSymbol}\;\text{\tt \char34 ()\char34}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

For primitive types we need to parse one identifier and \ensuremath{\Varid{prim}} wraps
this identifier into a \ensuremath{\Conid{TCons}}-structure with no type
...
In addition, we need to consider the unit type \ensuremath{()} as special
constructor. %
...
As example to assure the accuracy of this idea: \ensuremath{\Conid{IO}\;\Varid{a}} and \ensuremath{\Conid{Maybe}\;\Varid{a}} are type
...
\column{4}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{varParser}\mathbin{::}\Conid{Bool}\to \Conid{TypeExprParser}{}\<[E]%
\\
\>[B]{}\Varid{varParser}\;\Varid{inAListOrTuple}\mathrel{=}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{var}~\circledS~((\Varid{guard}\;\Varid{inAListOrTuple}\sequ {}\<[E]%
\\
\>[3]{}\hsindent{1}{}\<[4]%
\>[4]{}(\Varid{lexemer}\;\Varid{lower}<\!\!\!*~\Varid{notFollowedBy}\;\Varid{sigIdentifier})){}\<[E]%
\\
\>[3]{}\hsindent{1}{}\<[4]%
\>[4]{}~\rotatebox[origin=c]{90}{$\ominus$}~\Varid{lexemer}\;\Varid{lower}<\!\!\!*~\Varid{notFollowedBy}\;\Varid{anyChar}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

In the common case, a type variables is one lower-case character. %
Thus, we do not allow type variables like \ensuremath{\Conid{IO}\;\Varid{abc}}, the variable
parser fails on \ensuremath{\Varid{abc}} and our signature parser only reads \ensuremath{\Conid{IO}} as
primitive type. %
...
appear in the middle of a tuple like \ensuremath{(\Conid{Int},\Varid{a},\Conid{Int})} and hence, the
...
\\
\>[3]{}\hsindent{1}{}\<[4]%
\>[4]{}\circledast\Varid{sepBy1}\;(\Varid{signatureParser}\;\Conid{True})\;(\Varid{string}\;\text{\tt \char34 ,\char34}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\\
\>[B]{}[\mskip1.5mu [\mskip1.5mu \Conid{Infix}\;((\mathbin{\char92 \char95 }\to \Conid{FuncType})\circledS\Varid{lexemer}\;(\Varid{string}\;\text{\tt \char34 ->\char34}))\;\Conid{AssocRight}\mskip1.5mu]\mskip1.5mu]{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

Thus, when parsing a \ensuremath{\to } in an infix position, we return the partial
application of \ensuremath{\Conid{FuncType}}, because the two arguments of this
constructor are the type expression to the left and right of the
operator\footnote{\ensuremath{\Conid{Int}\to \Conid{Int}} => \ensuremath{\Conid{FuncType}\;\Conid{Int}\;\Conid{Int}}}. %
...
The other parse that we assign is \ensuremath{\Varid{signatureTerm}}. %
We need to parametrize \ensuremath{\Varid{signatureParser}} as well to indicate if we are
...
Therefore, we have to try \ensuremath{\Varid{tuplesParser}} first, thus, if it fails, the
