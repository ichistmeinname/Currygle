...
\\
\>[B]{}\Varid{p}\mathbin{<|>}\Varid{q}\mathrel{=}(\lambda \Varid{ts}\to \Varid{p}\;\Varid{ts}\plus \Varid{q}\;\Varid{ts}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\>[12]{}\lambda \Varid{ts}\to [\mskip1.5mu (\Varid{f}\;\Varid{x},\Varid{ts}_{1})\mid (\Varid{x},\Varid{ts}_{1})\leftarrow \Varid{p}\;\Varid{ts}\mskip1.5mu]{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
parentheses using \ensuremath{\mathbin{<|>}} and \ensuremath{\circledast}. 
...
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\mathbin{<\$>}\Varid{parseOpenParenthesis}\circledast\Varid{parseExpression}\circledast\Varid{parseCloseParenthesis}){}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
