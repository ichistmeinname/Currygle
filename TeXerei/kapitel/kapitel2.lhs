...
5|. Curry supports function definition with pattern matching, which is
often used in functional and logical programming languages. That means
that instantiated values like |True| and |False| can stand on the
...
language and |True| and |False| are the only possible values with
...
to constructors |False| and |True| with the type |Bool|. In this case
...
|ModuleInfo| and a list of |FunctionInfo| and |TypeInfo|. We define
...
\end{code} 
The syntax for lists is the same as in Haskell. |[]| is the empty
list, whereas |1:2:3:[]| is the same as |[1,2,3]| or |1:[2,3]|. The
...
|(a->b)| takes a value of an unspecific type and returns another
...
for boolean operators like the boolean equality |==|.  The second
approach, called narrowing, guesses a value for an unbound
value. Constraint operators like the boolean constraint |=.=| use
...
that use residuation. For example arithmetic (i.e. |+|, |-|, |*| etc)
